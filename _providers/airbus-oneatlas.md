---
api_count: 6
apis:
- description: OneAtlas Data Living Library is a platform that provides users with access to a diverse range of geospatial data and satellite imagery. The library is constantly updated with the latest data, ensuring
  name: OneAtlas Data Living Library
  slug: oneatlas-data-living-library
- description: 'The OneAtlas Data Pay-Per-Order API is a powerful tool that enables users to access and purchase high-resolution satellite imagery, elevation data, and other geospatial information on a pay-as-you-go '
  name: OneAtlas Data Pay-Per-Order API
  slug: oneatlas-data-pay-per-order-api
- description: 'The OneAtlas Data Pay-Per-Order Tasking API allows users to directly request high-resolution satellite imagery of specific locations at their convenience. With this API, users can easily select areas '
  name: OneAtlas Data Pay-Per-Order Tasking API
  slug: oneatlas-data-pay-per-order-tasking-api
- description: OneAtlas Basemap API is a geospatial service that provides access to high-resolution satellite imagery and basemaps. It allows users to retrieve up-to-date visual information of any location on Earth,
  name: OneAtlas Basemap API
  slug: oneatlas-basemap-api
- description: OneAtlas Radar API is a powerful tool that allows developers to access and integrate radar satellite data into their applications and processes. By utilizing this API, users can access real-time and h
  name: OneAtlas Radar API
  slug: oneatlas-radar-api
- description: The OneAtlas WorldDEM API is a tool that provides access to high-resolution global elevation data. This data, collected by satellites and processed using advanced technologies, offers a detailed and a
  name: OneAtlas WorldDEM API
  slug: oneatlas-worlddem-api
capabilities:
- description: ''
  name: Oneatlas Geospatial Data Access
  slug: oneatlas-geospatial-data-access
common:
- title: ''
  type: GettingStarted
  url: https://api.oneatlas.airbus.com/getting-started/
- title: ''
  type: Authentication
  url: https://api.oneatlas.airbus.com/getting-started/authentication/#authenticate
- title: ''
  type: Testing
  url: https://api.oneatlas.airbus.com/getting-started/api-testing-guide/
- title: ''
  type: Status
  url: https://status.oneatlas.airbus.com/
- title: ''
  type: ChangeLog
  url: https://api.oneatlas.airbus.com/releases/
- title: ''
  type: Portal
  url: https://api.oneatlas.airbus.com/
- title: ''
  type: GitHub
  url: https://github.com/airbusgeo
- title: ''
  type: LinkedIn
  url: https://www.linkedin.com/company/airbus
- title: ''
  type: Website
  url: https://www.airbus.com/
- title: ''
  type: SpectralRules
  url: rules/oneatlas-spectral-rules.yml
- title: ''
  type: NaftikoCapability
  url: capabilities/shared/oneatlas-api.yaml
- title: ''
  type: NaftikoCapability
  url: capabilities/oneatlas-geospatial-data-access.yaml
- title: ''
  type: Vocabulary
  url: vocabulary/oneatlas-vocabulary.yaml
- title: ''
  type: JSON-LD
  url: json-ld/oneatlas-context.jsonld
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-acquisition-date-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-activity-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-activity-search-options-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-activity-search-response-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-activity-small-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-activity-stage-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-activity-stages-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-api-key-page-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-api-key-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-associated-data-link-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-band-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-bbox-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-buffer-description-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-catalog-delete-expired-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-catalog-item-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-control-job-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-delete-operation-result-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-deleted-item-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-delivery-pneo-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-delivery-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-dem-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-describe-process-input-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-describe-process-links-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-describe-process-response-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-dimap-v2-packaging-parameters-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-email-notification-parameters-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-feasibility-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-features-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-footprint-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-format-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-geo-json-linestring-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-geo-json-multilinestring-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-geo-json-multipoint-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-geo-json-multipolygon-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-geo-json-point-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-geo-json-polygon-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-geojson-geometry-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-geometry-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-get-domain-response-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-histogram-response-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-human-readable-operation-summary-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-image-size-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-ingestion-job-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-link-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-md5-checksum-parameters-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-messagequeue-notification-parameters-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-metadata-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-notification-search-options-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-notification-search-response-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-notification-subscription-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-optical-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-pixel-coding-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-point-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-privilege-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-privilege-search-options-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-privilege-search-response-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-process-hal-catalog-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-process-hal-link-catalog-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-process-hal-link-common-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-process-hal-link-describe-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-process-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-processes-response-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-response-buffer-information-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-search-deleted-options-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-search-deleted-response-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-search-options-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-search-options-v2-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-search-response-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-segment-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-srs-expression-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-srs-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-step-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-target-workspace-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-tasking-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-update-options-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-update-response-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-webhook-notification-parameters-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-workspace-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-workspace-search-options-schema.json
- title: ''
  type: JSONSchema
  url: json-schema/oneatlas-workspace-search-response-schema.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-acquisition-date-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-activity-search-options-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-activity-search-response-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-activity-small-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-activity-stage-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-activity-stages-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-activity-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-api-key-page-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-api-key-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-associated-data-link-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-band-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-bbox-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-buffer-description-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-catalog-delete-expired-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-catalog-item-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-control-job-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-delete-operation-result-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-deleted-item-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-delivery-pneo-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-delivery-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-dem-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-describe-process-input-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-describe-process-links-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-describe-process-response-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-dimap-v2-packaging-parameters-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-email-notification-parameters-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-feasibility-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-features-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-footprint-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-format-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-geo-json-linestring-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-geo-json-multilinestring-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-geo-json-multipoint-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-geo-json-multipolygon-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-geo-json-point-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-geo-json-polygon-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-geojson-geometry-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-geometry-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-get-domain-response-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-histogram-response-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-human-readable-operation-summary-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-image-size-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-ingestion-job-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-link-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-md5-checksum-parameters-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-messagequeue-notification-parameters-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-metadata-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-notification-search-options-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-notification-search-response-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-notification-subscription-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-optical-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-pixel-coding-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-point-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-privilege-search-options-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-privilege-search-response-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-privilege-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-process-hal-catalog-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-process-hal-link-catalog-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-process-hal-link-common-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-process-hal-link-describe-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-process-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-processes-response-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-response-buffer-information-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-search-deleted-options-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-search-deleted-response-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-search-options-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-search-options-v2-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-search-response-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-segment-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-srs-expression-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-srs-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-step-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-target-workspace-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-tasking-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-update-options-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-update-response-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-webhook-notification-parameters-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-workspace-search-options-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-workspace-search-response-structure.json
- title: ''
  type: JSONStructure
  url: json-structure/oneatlas-workspace-structure.json
- title: ''
  type: Example
  url: examples/oneatlas-acquisition-date-example.json
- title: ''
  type: Example
  url: examples/oneatlas-activity-example.json
- title: ''
  type: Example
  url: examples/oneatlas-activity-search-options-example.json
- title: ''
  type: Example
  url: examples/oneatlas-activity-search-response-example.json
- title: ''
  type: Example
  url: examples/oneatlas-activity-small-example.json
- title: ''
  type: Example
  url: examples/oneatlas-activity-stage-example.json
- title: ''
  type: Example
  url: examples/oneatlas-activity-stages-example.json
- title: ''
  type: Example
  url: examples/oneatlas-api-key-example.json
- title: ''
  type: Example
  url: examples/oneatlas-api-key-page-example.json
- title: ''
  type: Example
  url: examples/oneatlas-associated-data-link-example.json
- title: ''
  type: Example
  url: examples/oneatlas-band-example.json
- title: ''
  type: Example
  url: examples/oneatlas-bbox-example.json
- title: ''
  type: Example
  url: examples/oneatlas-buffer-description-example.json
- title: ''
  type: Example
  url: examples/oneatlas-catalog-delete-expired-example.json
- title: ''
  type: Example
  url: examples/oneatlas-catalog-item-example.json
- title: ''
  type: Example
  url: examples/oneatlas-control-job-example.json
- title: ''
  type: Example
  url: examples/oneatlas-delete-operation-result-example.json
- title: ''
  type: Example
  url: examples/oneatlas-deleted-item-example.json
- title: ''
  type: Example
  url: examples/oneatlas-delivery-example.json
- title: ''
  type: Example
  url: examples/oneatlas-delivery-pneo-example.json
- title: ''
  type: Example
  url: examples/oneatlas-dem-example.json
- title: ''
  type: Example
  url: examples/oneatlas-describe-process-input-example.json
- title: ''
  type: Example
  url: examples/oneatlas-describe-process-links-example.json
- title: ''
  type: Example
  url: examples/oneatlas-describe-process-response-example.json
- title: ''
  type: Example
  url: examples/oneatlas-dimap-v2-packaging-parameters-example.json
- title: ''
  type: Example
  url: examples/oneatlas-email-notification-parameters-example.json
- title: ''
  type: Example
  url: examples/oneatlas-feasibility-example.json
- title: ''
  type: Example
  url: examples/oneatlas-features-example.json
- title: ''
  type: Example
  url: examples/oneatlas-footprint-example.json
- title: ''
  type: Example
  url: examples/oneatlas-format-example.json
- title: ''
  type: Example
  url: examples/oneatlas-geo-json-linestring-example.json
- title: ''
  type: Example
  url: examples/oneatlas-geo-json-multilinestring-example.json
- title: ''
  type: Example
  url: examples/oneatlas-geo-json-multipoint-example.json
- title: ''
  type: Example
  url: examples/oneatlas-geo-json-multipolygon-example.json
- title: ''
  type: Example
  url: examples/oneatlas-geo-json-point-example.json
- title: ''
  type: Example
  url: examples/oneatlas-geo-json-polygon-example.json
- title: ''
  type: Example
  url: examples/oneatlas-geojson-geometry-example.json
- title: ''
  type: Example
  url: examples/oneatlas-geometry-example.json
- title: ''
  type: Example
  url: examples/oneatlas-get-domain-response-example.json
- title: ''
  type: Example
  url: examples/oneatlas-histogram-response-example.json
- title: ''
  type: Example
  url: examples/oneatlas-human-readable-operation-summary-example.json
- title: ''
  type: Example
  url: examples/oneatlas-image-size-example.json
- title: ''
  type: Example
  url: examples/oneatlas-ingestion-job-example.json
- title: ''
  type: Example
  url: examples/oneatlas-link-example.json
- title: ''
  type: Example
  url: examples/oneatlas-md5-checksum-parameters-example.json
- title: ''
  type: Example
  url: examples/oneatlas-messagequeue-notification-parameters-example.json
- title: ''
  type: Example
  url: examples/oneatlas-metadata-example.json
- title: ''
  type: Example
  url: examples/oneatlas-notification-search-options-example.json
- title: ''
  type: Example
  url: examples/oneatlas-notification-search-response-example.json
- title: ''
  type: Example
  url: examples/oneatlas-notification-subscription-example.json
- title: ''
  type: Example
  url: examples/oneatlas-optical-example.json
- title: ''
  type: Example
  url: examples/oneatlas-pixel-coding-example.json
- title: ''
  type: Example
  url: examples/oneatlas-point-example.json
- title: ''
  type: Example
  url: examples/oneatlas-privilege-example.json
- title: ''
  type: Example
  url: examples/oneatlas-privilege-search-options-example.json
- title: ''
  type: Example
  url: examples/oneatlas-privilege-search-response-example.json
- title: ''
  type: Example
  url: examples/oneatlas-process-example.json
- title: ''
  type: Example
  url: examples/oneatlas-process-hal-catalog-example.json
- title: ''
  type: Example
  url: examples/oneatlas-process-hal-link-catalog-example.json
- title: ''
  type: Example
  url: examples/oneatlas-process-hal-link-common-example.json
- title: ''
  type: Example
  url: examples/oneatlas-process-hal-link-describe-example.json
- title: ''
  type: Example
  url: examples/oneatlas-processes-response-example.json
- title: ''
  type: Example
  url: examples/oneatlas-response-buffer-information-example.json
- title: ''
  type: Example
  url: examples/oneatlas-search-deleted-options-example.json
- title: ''
  type: Example
  url: examples/oneatlas-search-deleted-response-example.json
- title: ''
  type: Example
  url: examples/oneatlas-search-options-example.json
- title: ''
  type: Example
  url: examples/oneatlas-search-options-v2-example.json
- title: ''
  type: Example
  url: examples/oneatlas-search-response-example.json
- title: ''
  type: Example
  url: examples/oneatlas-segment-example.json
- title: ''
  type: Example
  url: examples/oneatlas-srs-example.json
- title: ''
  type: Example
  url: examples/oneatlas-srs-expression-example.json
- title: ''
  type: Example
  url: examples/oneatlas-step-example.json
- title: ''
  type: Example
  url: examples/oneatlas-target-workspace-example.json
- title: ''
  type: Example
  url: examples/oneatlas-tasking-example.json
- title: ''
  type: Example
  url: examples/oneatlas-update-options-example.json
- title: ''
  type: Example
  url: examples/oneatlas-update-response-example.json
- title: ''
  type: Example
  url: examples/oneatlas-webhook-notification-parameters-example.json
- title: ''
  type: Example
  url: examples/oneatlas-workspace-example.json
- title: ''
  type: Example
  url: examples/oneatlas-workspace-search-options-example.json
- title: ''
  type: Example
  url: examples/oneatlas-workspace-search-response-example.json
created: '2025-02-06'
description: Airbus OneAtlas is the commercial geospatial platform from Airbus Defence and Space, providing developer APIs for accessing high-resolution satellite imagery, elevation data, basemaps, and radar data. The platform covers optical imagery via the Living Library and Pay-Per-Order services, satellite tasking for new acquisitions, WorldDEM global elevation data, SAR radar imagery, and seamless basemap tiles.
features: []
image: https://kinlane-productions.s3.amazonaws.com/apis-json/apis-json-logo.jpg
integrations: []
jsonld:
- class_count: 0
  name: Oneatlas Context
  property_count: 18
  slug: oneatlas-context
layout: provider
modified: '2026-04-19'
name: Airbus OneAtlas
rules:
- name: Airbus OneAtlas API Rules
  rule_count: 12
  severity_counts:
    error: 4
    hint: 0
    info: 0
    warn: 8
  slug: oneatlas-spectral-rules
skills: []
slug: airbus-oneatlas
solutions: []
source_yaml: "aid: airbus-oneatlas\nurl: >-\n  https://raw.githubusercontent.com/api-evangelist/airbus-oneatlas/refs/heads/main/apis.yml\napis:\n- aid: airbus-oneatlas:oneatlas-data-living-library\n  name: OneAtlas Data Living Library\n  tags:\n  - Airbus\n  - Geospatial\n  - Imagery\n  - Remote Sensing\n  - Satellites\n  - Satellite Imagery\n  - Streaming\n  humanURL: >-\n    https://api.oneatlas.airbus.com/api-catalog-v2/oad-living-library/overview/index.html\n  properties:\n  - url: >-\n      https://api.oneatlas.airbus.com/api-catalog-v2/oad-living-library/tutorials/\n    name: OneAtlas Data - Living Library\n    type: Tutorials\n    description: 'null'\n  - url: >-\n      https://api.oneatlas.airbus.com/api-catalog-v2/oad-living-library/notebooks/\n    name: OneAtlas Data Living Library Notebooks & Postman collections\n    type: PostmanCollection\n    description: 'null'\n  - url: >-\n      https://api.oneatlas.airbus.com/api-catalog-v2/oad-living-library/notebooks/\n    name: OneAtlas\
  \ Data Living Library Notebooks & Postman collections\n    type: JupyterNotebook\n    description: 'null'\n  - url: https://api.oneatlas.airbus.com/releases/#oad-living-library\n    name: Releases\n    type: ChangeLog\n    description: 'null'\n  description: >-\n    OneAtlas Data Living Library is a platform that provides users with access to\n    a diverse range of geospatial data and satellite imagery. The library is constantly\n    updated with the latest data, ensuring that users have access to accurate and\n    timely information for their projects. Users can easily search for and find\n    the data they need, whether it be for environmental monitoring, urban planning,\n    agriculture, or any other application.\n  features:\n  - Search and stream satellite imagery from a continuously updated library\n  - Filter by date range, cloud cover, and area of interest\n  - Access optical imagery from Pleiades, SPOT, and third-party satellites\n  - Jupyter notebook and Postman collection support\n\
  - aid: airbus-oneatlas:oneatlas-data-pay-per-order-api\n  name: OneAtlas Data Pay-Per-Order API\n  tags:\n  - Airbus\n  - Archives\n  - Geospatial\n  - Imagery\n  - Pay-Per-Order\n  - Satellites\n  - Satellite Imagery\n  humanURL: >-\n    https://api.oneatlas.airbus.com/api-catalog-v2/oad-ppo-archives/overview/index.html\n  properties:\n  - url: >-\n      https://api.oneatlas.airbus.com/api-catalog-v2/oad-ppo-archives/tutorials/\n    name: OneAtlas Data - Pay-Per-Order Archives\n    type: Tutorials\n    description: 'null'\n  - url: >-\n      https://api.oneatlas.airbus.com/api-catalog-v2/oad-ppo-archives/notebooks/\n    name: OneAtlas Data pay-per-order archives Notebooks & Postman collections\n    type: PostmanCollection\n    description: 'null'\n  - url: >-\n      https://api.oneatlas.airbus.com/api-catalog-v2/oad-ppo-archives/notebooks/\n    name: OneAtlas Data pay-per-order archives Notebooks & Postman collections\n    type: JupyterNotebook\n    description: 'null'\n  - url: https://api.oneatlas.airbus.com/releases/#oad-pay-per-order-archives\n\
  \    name: Releases\n    type: ChangeLog\n    description: 'null'\n  description: >-\n    The OneAtlas Data Pay-Per-Order API is a powerful tool that enables users to\n    access and purchase high-resolution satellite imagery, elevation data, and other\n    geospatial information on a pay-as-you-go basis. By integrating this API into\n    their applications or systems, users can quickly and easily obtain the precise\n    data they need for their projects without committing to long-term subscriptions\n    or contracts.\n  features:\n  - Purchase and download archived satellite imagery on demand\n  - Pay-as-you-go pricing with no subscription commitment\n  - Access to historical imagery archive with global coverage\n  - Multiple delivery formats and resolution options\n- aid: airbus-oneatlas:oneatlas-data-pay-per-order-tasking-api\n  name: OneAtlas Data Pay-Per-Order Tasking API\n  tags:\n  - Airbus\n  - Geospatial\n  - Imagery\n  - Pay-Per-Order\n  - Satellites\n  - Satellite Imagery\n\
  \  - Tasking\n  humanURL: >-\n    https://api.oneatlas.airbus.com/api-catalog-v2/oad-ppo-tasking/overview/index.html\n  properties:\n  - url: >-\n      https://api.oneatlas.airbus.com/api-catalog-v2/oad-ppo-tasking/tutorials/\n    name: OneAtlas Data - Tasking\n    type: Tutorials\n    description: 'null'\n  - url: >-\n      https://api.oneatlas.airbus.com/api-catalog-v2/oad-ppo-tasking/notebooks/\n    name: OneAtlas Data pay-per-order tasking Notebooks & Postman collections\n    type: PostmanCollection\n    description: 'null'\n  - url: >-\n      https://api.oneatlas.airbus.com/api-catalog-v2/oad-ppo-tasking/notebooks/\n    name: OneAtlas Data pay-per-order tasking Notebooks & Postman collections\n    type: JupyterNotebook\n    description: 'null'\n  - url: https://api.oneatlas.airbus.com/releases/#oad-pay-per-order-tasking\n    name: Releases\n    type: ChangeLog\n    description: 'null'\n  description: >-\n    The OneAtlas Data Pay-Per-Order Tasking API allows users to directly request\n\
  \    high-resolution satellite imagery of specific locations at their convenience.\n    With this API, users can easily select areas of interest, order and pay for\n    the data they need, and receive the requested imagery quickly and efficiently.\n    This tool enables users to access up-to-date satellite data for a wide range\n    of applications, such as environmental monitoring, agriculture, urban planning,\n    and disaster response.\n  features:\n  - Commission new satellite imagery acquisitions of specific locations\n  - Select acquisition parameters including resolution and urgency\n  - Track tasking order status and estimated delivery\n  - Access to Pleiades Neo and Pleiades satellite fleet\n- aid: airbus-oneatlas:oneatlas-basemap-api\n  name: OneAtlas Basemap API\n  tags:\n  - Airbus\n  - Basemap\n  - Geospatial\n  - Imagery\n  - Maps\n  - Satellites\n  - Tiles\n  humanURL: >-\n    https://api.oneatlas.airbus.com/api-catalog-v2/oad-basemap/overview/index.html\n  properties:\n\
  \  - url: >-\n      https://api.oneatlas.airbus.com/api-catalog-v2/oad-basemap/tutorials/#basemap-access\n    name: OneAtlas Basemap - Tutorials\n    type: Tutorials\n    description: 'null'\n  - url: https://api.oneatlas.airbus.com/api-catalog-v2/oad-basemap/notebooks/\n    name: OneAtlas Data Basemap Notebooks & Postman collections\n    type: PostmanCollection\n    description: 'null'\n  - url: https://api.oneatlas.airbus.com/api-catalog-v2/oad-basemap/notebooks/\n    name: OneAtlas Data Basemap Notebooks & Postman collections\n    type: JupyterNotebook\n    description: 'null'\n  - url: https://api.oneatlas.airbus.com/releases/#oad-basemap\n    name: Releases\n    type: ChangeLog\n    description: 'null'\n  description: >-\n    OneAtlas Basemap API is a geospatial service that provides access to high-resolution\n    satellite imagery and basemaps. It allows users to retrieve up-to-date visual\n    information of any location on Earth, enabling them to analyze and monitor changes\n \
  \   in the environment, infrastructure, and land use.\n  features:\n  - Access global seamless basemap tiles built from Airbus satellite imagery\n  - Standard XYZ tile service compatible with mapping libraries\n  - Multiple basemap styles and zoom levels\n- aid: airbus-oneatlas:oneatlas-radar-api\n  name: OneAtlas Radar API\n  tags:\n  - Airbus\n  - Geospatial\n  - Imagery\n  - Radar\n  - SAR\n  - Satellites\n  - Tasking\n  humanURL: https://api.oneatlas.airbus.com/api-catalog-v2/radar/overview/index.html\n  properties:\n  - url: https://api.oneatlas.airbus.com/api-catalog-v2/radar/tutorials/\n    name: Radar Tasking & Archive - API\n    type: Tutorials\n    description: 'null'\n  - url: https://api.oneatlas.airbus.com/api-catalog-v2/radar/notebooks/\n    name: Radar Notebooks & Postman collections\n    type: JupyterNotebook\n    description: 'null'\n  - url: https://api.oneatlas.airbus.com/api-catalog-v2/radar/notebooks/\n    name: Radar Notebooks & Postman collections\n    type: PostmanCollection\n\
  \    description: 'null'\n  - url: https://api.oneatlas.airbus.com/releases/#radar\n    name: Releases\n    type: ChangeLog\n    description: 'null'\n  description: >-\n    OneAtlas Radar API is a powerful tool that allows developers to access and integrate\n    radar satellite data into their applications and processes. By utilizing this\n    API, users can access real-time and historical radar imagery, which can be used\n    for a wide range of applications including agriculture, disaster management,\n    urban planning, and security.\n  features:\n  - Access SAR (synthetic aperture radar) imagery from Airbus radar satellites\n  - Supports both archive search and new tasking acquisitions\n  - All-weather, day-night imaging capability\n  - Multiple polarization modes and resolutions\n- aid: airbus-oneatlas:oneatlas-worlddem-api\n  name: OneAtlas WorldDEM API\n  tags:\n  - Airbus\n  - Digital Elevation Model\n  - Elevation\n  - Geospatial\n  - Imagery\n  - Satellites\n  - Terrain\n  humanURL:\
  \ >-\n    https://api.oneatlas.airbus.com/api-catalog-v2/worlddem/overview/index.html\n  properties:\n  - url: https://api.oneatlas.airbus.com/api-catalog-v2/worlddem/tutorials/\n    name: WorldDEM - API\n    type: Tutorials\n    description: 'null'\n  - url: https://api.oneatlas.airbus.com/api-catalog-v2/worlddem/notebooks/\n    name: WorldDEM Notebooks & Postman collections\n    type: JupyterNotebook\n    description: 'null'\n  - url: https://api.oneatlas.airbus.com/api-catalog-v2/worlddem/notebooks/\n    name: WorldDEM Notebooks & Postman collections\n    type: PostmanCollection\n    description: 'null'\n  - url: https://api.oneatlas.airbus.com/releases/#worlddem\n    name: Releases\n    type: ChangeLog\n    description: 'null'\n  - url: properties/oneatlas-worlddem-api-openapi.yml\n    type: OpenAPI\n  description: >-\n    The OneAtlas WorldDEM API is a tool that provides access to high-resolution\n    global elevation data. This data, collected by satellites and processed using\n\
  \    advanced technologies, offers a detailed and accurate representation of the\n    Earth's surface. By integrating the WorldDEM API into their applications, users\n    can benefit from precise terrain information for a wide range of use cases,\n    such as urban planning, disaster response, agriculture, and infrastructure development.\n  features:\n  - Access global digital elevation model data at up to 12m resolution\n  - Derived from TanDEM-X satellite radar interferometry\n  - Point elevation queries and area elevation profiles\n  - Used for terrain analysis, flood modeling, and infrastructure planning\nname: Airbus OneAtlas\ntags:\n- Imagery\n- Satellites\ntype: Index\nimage: https://kinlane-productions.s3.amazonaws.com/apis-json/apis-json-logo.jpg\naccess: 3rd-Party\ncommon:\n- url: https://api.oneatlas.airbus.com/getting-started/\n  name: Getting started\n  type: GettingStarted\n  description: 'null'\n- url: >-\n    https://api.oneatlas.airbus.com/getting-started/authentication/#authenticate\n\
  \  name: OneAtlas - Authentication\n  type: Authentication\n  description: 'null'\n- url: https://api.oneatlas.airbus.com/getting-started/api-testing-guide/\n  name: APIs overview\n  type: Testing\n  description: 'null'\n- url: https://status.oneatlas.airbus.com/\n  name: Status Page OneAtlas\n  type: Status\n  description: 'null'\n- url: https://api.oneatlas.airbus.com/releases/\n  name: Releases\n  type: ChangeLog\n  description: 'null'\n- url: https://api.oneatlas.airbus.com/\n  name: Airbus - OneAtlas Developer Portal\n  type: Portal\n  description: 'null'\n- type: GitHub\n  url: https://github.com/airbusgeo\n- type: LinkedIn\n  url: https://www.linkedin.com/company/airbus\n- type: Website\n  url: https://www.airbus.com/\n- type: SpectralRules\n  url: rules/oneatlas-spectral-rules.yml\n- type: NaftikoCapability\n  url: capabilities/shared/oneatlas-api.yaml\n- type: NaftikoCapability\n  url: capabilities/oneatlas-geospatial-data-access.yaml\n- type: Vocabulary\n  url: vocabulary/oneatlas-vocabulary.yaml\n\
  - type: JSON-LD\n  url: json-ld/oneatlas-context.jsonld\n- type: JSONSchema\n  url: json-schema/oneatlas-acquisition-date-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-activity-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-activity-search-options-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-activity-search-response-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-activity-small-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-activity-stage-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-activity-stages-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-api-key-page-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-api-key-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-associated-data-link-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-band-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-bbox-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-buffer-description-schema.json\n\
  - type: JSONSchema\n  url: json-schema/oneatlas-catalog-delete-expired-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-catalog-item-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-control-job-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-delete-operation-result-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-deleted-item-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-delivery-pneo-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-delivery-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-dem-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-describe-process-input-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-describe-process-links-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-describe-process-response-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-dimap-v2-packaging-parameters-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-email-notification-parameters-schema.json\n\
  - type: JSONSchema\n  url: json-schema/oneatlas-feasibility-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-features-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-footprint-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-format-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-geo-json-linestring-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-geo-json-multilinestring-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-geo-json-multipoint-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-geo-json-multipolygon-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-geo-json-point-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-geo-json-polygon-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-geojson-geometry-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-geometry-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-get-domain-response-schema.json\n- type:\
  \ JSONSchema\n  url: json-schema/oneatlas-histogram-response-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-human-readable-operation-summary-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-image-size-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-ingestion-job-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-link-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-md5-checksum-parameters-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-messagequeue-notification-parameters-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-metadata-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-notification-search-options-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-notification-search-response-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-notification-subscription-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-optical-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-pixel-coding-schema.json\n\
  - type: JSONSchema\n  url: json-schema/oneatlas-point-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-privilege-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-privilege-search-options-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-privilege-search-response-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-process-hal-catalog-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-process-hal-link-catalog-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-process-hal-link-common-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-process-hal-link-describe-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-process-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-processes-response-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-response-buffer-information-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-search-deleted-options-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-search-deleted-response-schema.json\n\
  - type: JSONSchema\n  url: json-schema/oneatlas-search-options-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-search-options-v2-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-search-response-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-segment-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-srs-expression-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-srs-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-step-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-target-workspace-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-tasking-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-update-options-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-update-response-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-webhook-notification-parameters-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-workspace-schema.json\n- type: JSONSchema\n  url: json-schema/oneatlas-workspace-search-options-schema.json\n\
  - type: JSONSchema\n  url: json-schema/oneatlas-workspace-search-response-schema.json\n- type: JSONStructure\n  url: json-structure/oneatlas-acquisition-date-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-activity-search-options-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-activity-search-response-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-activity-small-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-activity-stage-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-activity-stages-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-activity-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-api-key-page-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-api-key-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-associated-data-link-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-band-structure.json\n\
  - type: JSONStructure\n  url: json-structure/oneatlas-bbox-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-buffer-description-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-catalog-delete-expired-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-catalog-item-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-control-job-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-delete-operation-result-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-deleted-item-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-delivery-pneo-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-delivery-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-dem-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-describe-process-input-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-describe-process-links-structure.json\n\
  - type: JSONStructure\n  url: json-structure/oneatlas-describe-process-response-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-dimap-v2-packaging-parameters-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-email-notification-parameters-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-feasibility-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-features-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-footprint-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-format-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-geo-json-linestring-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-geo-json-multilinestring-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-geo-json-multipoint-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-geo-json-multipolygon-structure.json\n- type: JSONStructure\n\
  \  url: json-structure/oneatlas-geo-json-point-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-geo-json-polygon-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-geojson-geometry-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-geometry-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-get-domain-response-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-histogram-response-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-human-readable-operation-summary-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-image-size-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-ingestion-job-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-link-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-md5-checksum-parameters-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-messagequeue-notification-parameters-structure.json\n\
  - type: JSONStructure\n  url: json-structure/oneatlas-metadata-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-notification-search-options-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-notification-search-response-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-notification-subscription-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-optical-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-pixel-coding-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-point-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-privilege-search-options-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-privilege-search-response-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-privilege-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-process-hal-catalog-structure.json\n- type: JSONStructure\n  url:\
  \ json-structure/oneatlas-process-hal-link-catalog-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-process-hal-link-common-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-process-hal-link-describe-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-process-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-processes-response-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-response-buffer-information-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-search-deleted-options-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-search-deleted-response-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-search-options-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-search-options-v2-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-search-response-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-segment-structure.json\n\
  - type: JSONStructure\n  url: json-structure/oneatlas-srs-expression-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-srs-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-step-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-target-workspace-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-tasking-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-update-options-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-update-response-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-webhook-notification-parameters-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-workspace-search-options-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-workspace-search-response-structure.json\n- type: JSONStructure\n  url: json-structure/oneatlas-workspace-structure.json\n- type: Example\n  url: examples/oneatlas-acquisition-date-example.json\n\
  - type: Example\n  url: examples/oneatlas-activity-example.json\n- type: Example\n  url: examples/oneatlas-activity-search-options-example.json\n- type: Example\n  url: examples/oneatlas-activity-search-response-example.json\n- type: Example\n  url: examples/oneatlas-activity-small-example.json\n- type: Example\n  url: examples/oneatlas-activity-stage-example.json\n- type: Example\n  url: examples/oneatlas-activity-stages-example.json\n- type: Example\n  url: examples/oneatlas-api-key-example.json\n- type: Example\n  url: examples/oneatlas-api-key-page-example.json\n- type: Example\n  url: examples/oneatlas-associated-data-link-example.json\n- type: Example\n  url: examples/oneatlas-band-example.json\n- type: Example\n  url: examples/oneatlas-bbox-example.json\n- type: Example\n  url: examples/oneatlas-buffer-description-example.json\n- type: Example\n  url: examples/oneatlas-catalog-delete-expired-example.json\n- type: Example\n  url: examples/oneatlas-catalog-item-example.json\n- type:\
  \ Example\n  url: examples/oneatlas-control-job-example.json\n- type: Example\n  url: examples/oneatlas-delete-operation-result-example.json\n- type: Example\n  url: examples/oneatlas-deleted-item-example.json\n- type: Example\n  url: examples/oneatlas-delivery-example.json\n- type: Example\n  url: examples/oneatlas-delivery-pneo-example.json\n- type: Example\n  url: examples/oneatlas-dem-example.json\n- type: Example\n  url: examples/oneatlas-describe-process-input-example.json\n- type: Example\n  url: examples/oneatlas-describe-process-links-example.json\n- type: Example\n  url: examples/oneatlas-describe-process-response-example.json\n- type: Example\n  url: examples/oneatlas-dimap-v2-packaging-parameters-example.json\n- type: Example\n  url: examples/oneatlas-email-notification-parameters-example.json\n- type: Example\n  url: examples/oneatlas-feasibility-example.json\n- type: Example\n  url: examples/oneatlas-features-example.json\n- type: Example\n  url: examples/oneatlas-footprint-example.json\n\
  - type: Example\n  url: examples/oneatlas-format-example.json\n- type: Example\n  url: examples/oneatlas-geo-json-linestring-example.json\n- type: Example\n  url: examples/oneatlas-geo-json-multilinestring-example.json\n- type: Example\n  url: examples/oneatlas-geo-json-multipoint-example.json\n- type: Example\n  url: examples/oneatlas-geo-json-multipolygon-example.json\n- type: Example\n  url: examples/oneatlas-geo-json-point-example.json\n- type: Example\n  url: examples/oneatlas-geo-json-polygon-example.json\n- type: Example\n  url: examples/oneatlas-geojson-geometry-example.json\n- type: Example\n  url: examples/oneatlas-geometry-example.json\n- type: Example\n  url: examples/oneatlas-get-domain-response-example.json\n- type: Example\n  url: examples/oneatlas-histogram-response-example.json\n- type: Example\n  url: examples/oneatlas-human-readable-operation-summary-example.json\n- type: Example\n  url: examples/oneatlas-image-size-example.json\n- type: Example\n  url: examples/oneatlas-ingestion-job-example.json\n\
  - type: Example\n  url: examples/oneatlas-link-example.json\n- type: Example\n  url: examples/oneatlas-md5-checksum-parameters-example.json\n- type: Example\n  url: examples/oneatlas-messagequeue-notification-parameters-example.json\n- type: Example\n  url: examples/oneatlas-metadata-example.json\n- type: Example\n  url: examples/oneatlas-notification-search-options-example.json\n- type: Example\n  url: examples/oneatlas-notification-search-response-example.json\n- type: Example\n  url: examples/oneatlas-notification-subscription-example.json\n- type: Example\n  url: examples/oneatlas-optical-example.json\n- type: Example\n  url: examples/oneatlas-pixel-coding-example.json\n- type: Example\n  url: examples/oneatlas-point-example.json\n- type: Example\n  url: examples/oneatlas-privilege-example.json\n- type: Example\n  url: examples/oneatlas-privilege-search-options-example.json\n- type: Example\n  url: examples/oneatlas-privilege-search-response-example.json\n- type: Example\n  url: examples/oneatlas-process-example.json\n\
  - type: Example\n  url: examples/oneatlas-process-hal-catalog-example.json\n- type: Example\n  url: examples/oneatlas-process-hal-link-catalog-example.json\n- type: Example\n  url: examples/oneatlas-process-hal-link-common-example.json\n- type: Example\n  url: examples/oneatlas-process-hal-link-describe-example.json\n- type: Example\n  url: examples/oneatlas-processes-response-example.json\n- type: Example\n  url: examples/oneatlas-response-buffer-information-example.json\n- type: Example\n  url: examples/oneatlas-search-deleted-options-example.json\n- type: Example\n  url: examples/oneatlas-search-deleted-response-example.json\n- type: Example\n  url: examples/oneatlas-search-options-example.json\n- type: Example\n  url: examples/oneatlas-search-options-v2-example.json\n- type: Example\n  url: examples/oneatlas-search-response-example.json\n- type: Example\n  url: examples/oneatlas-segment-example.json\n- type: Example\n  url: examples/oneatlas-srs-example.json\n- type: Example\n  url:\
  \ examples/oneatlas-srs-expression-example.json\n- type: Example\n  url: examples/oneatlas-step-example.json\n- type: Example\n  url: examples/oneatlas-target-workspace-example.json\n- type: Example\n  url: examples/oneatlas-tasking-example.json\n- type: Example\n  url: examples/oneatlas-update-options-example.json\n- type: Example\n  url: examples/oneatlas-update-response-example.json\n- type: Example\n  url: examples/oneatlas-webhook-notification-parameters-example.json\n- type: Example\n  url: examples/oneatlas-workspace-example.json\n- type: Example\n  url: examples/oneatlas-workspace-search-options-example.json\n- type: Example\n  url: examples/oneatlas-workspace-search-response-example.json\ncreated: '2025-02-06'\nmodified: '2026-04-19'\nposition: Consumer\ndescription: |-\n  Airbus OneAtlas is the commercial geospatial platform from Airbus Defence and Space, providing developer APIs for accessing high-resolution satellite imagery, elevation data, basemaps, and radar data. The platform\
  \ covers optical imagery via the Living Library and Pay-Per-Order services, satellite tasking for new acquisitions, WorldDEM global elevation data, SAR radar imagery, and seamless basemap tiles.\nmaintainers:\n- FN: Kin Lane\n  email: kin@apievangelist.com\nspecificationVersion: '0.19'\n"
source_yaml_url: https://raw.githubusercontent.com/api-evangelist/airbus-oneatlas/refs/heads/main/apis.yml
tags:
- Imagery
- Satellites
url: https://raw.githubusercontent.com/api-evangelist/airbus-oneatlas/refs/heads/main/apis.yml
use_cases: []
---
